...
function saveNewUser(form) {
  ...
  flag = textInputValidation(form.elements.email, flag);
  flag = textInputValidation(form.elements.phone, flag);
  if (flag) {
    let name = $(form.elements.name).val().trim(),
      birthYear = $(form.elements.birthday).val().trim(),
      email = $(form.elements.email).val().trim(),
    ...
    $.ajax({
      ...
      data: JSON.stringify({ name, birthYear, email, phone }),
...
function editBtn(user, tr) {
  ...
  a.on("click", function () {
    ...
    $(edit.elements.birthday).val(user.birthYear);
    $(edit.elements.email).val(user.email);
